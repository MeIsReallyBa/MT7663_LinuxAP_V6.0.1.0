	...
	UCHAR op_mode)
{
	...
	struct wifi_dev_ops *ops = wdev->wdev_ops;
#ifdef VLAN_SUPPORT
	UCHAR byte0, byte1, extra_field_offset = 2 * ETH_ALEN;
#endif /*VLAN_SUPPORT*/

#ifdef VLAN_SUPPORT
	/*For IOT, remove unused fields*/
	if (wdev && GET_OS_PKT_DATAPTR(pPacket)[12] == 0x81 && GET_OS_PKT_DATAPTR(pPacket)[13] == 0x00) {
		UINT16 tci = (GET_OS_PKT_DATAPTR(pPacket)[14]<<8) | (GET_OS_PKT_DATAPTR(pPacket)[15]);
		UINT16 vid = tci & MASK_TCI_VID;

		extra_field_offset = 2 * ETH_ALEN+LENGTH_802_1Q;
	...
	byte0 = GET_OS_PKT_DATAPTR(pPacket)[extra_field_offset];
	byte1 = GET_OS_PKT_DATAPTR(pPacket)[extra_field_offset+1];

	/*If there is the extra field, remove it*/
	if (((byte0<<8) | byte1) == GET_OS_PKT_LEN(pPacket) - extra_field_offset - 2) { /*2 : len of extra field*/
		memmove(GET_OS_PKT_DATAPTR(pPacket) + 2, GET_OS_PKT_DATAPTR(pPacket), extra_field_offset);
		skb_pull_rcsum(pPacket, 2);
		skb_reset_network_header(pPacket);
		skb_reset_transport_header(pPacket);
		skb_reset_mac_len(pPacket);
	}
