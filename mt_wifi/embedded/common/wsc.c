	...
	IN  PWSC_CTRL pWscControl)
{
	...
	if (CurOpMode == STA_MODE) {
		...
		rtmp_set_channel(pAd, wdev, pAd->StaCfg[apcli_idx].MlmeAux.Channel);
#else
		RTEnqueueInternalCmd(pAd, CMDTHREAD_APCLI_PBC_TIMEOUT, (VOID *)&pAd->StaCfg[apcli_idx], sizeof(STA_ADMIN_CONFIG));
#endif
		/* bring apcli interface down first */
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				 ("!!! WscPBCExec --> Goto CMDTHREAD_APCLI_IF_DOWN!, pAd->StaCfg[%d].Enable=%d  !!!\n",
				  apcli_idx,
				  pAd->StaCfg[apcli_idx].ApcliInfStat.Enable));
		{
